...
def draw_layers(layers, width, height, colour_palette_keyword):
    ...
    elif len(colour_dict) < len(layers) + 1:
...
def main(num):
    ...
    colour_theme = theme or None
    landscape = draw_layers(
        [layer_4, layer_3, layer_2, layer_1], width, height, colour_theme
    )

    landscape.save(f"{os.getcwd()}\\Image {num}.png")


if __name__ == "__main__":
    for i in range(11):
        theme = random.choice(["river", "forest", "sunset", "sunset", "sunset","rainbow","Blue","Let_them_eat_cake","I_demand_a_Pancake","Ocean_five","clairedelune","you_are_beautiful","Compatible"])
        #print(i,theme)
        main(i)